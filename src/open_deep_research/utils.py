...
async def summarize_webpage(model: BaseChatModel, webpage_content: str) -> str:
    """Summarize webpage content using AI model with timeout and retry protection.

    This function is intentionally defensive:
    - It enforces a per-call timeout to avoid hangs on slow providers.
    - It supports a small number of retry attempts, controlled via env vars.
    - On persistent failure it falls back to returning the original content.

    Environment variables (optional):
        SUMMARIZATION_TIMEOUT_SECONDS: per-attempt timeout (default: 60)
        SUMMARIZATION_MAX_ATTEMPTS:   number of attempts (default: 1)
    ...
    try:
        timeout_seconds = float(os.getenv("SUMMARIZATION_TIMEOUT_SECONDS", "60"))
    ...
    try:
        max_attempts = int(os.getenv("SUMMARIZATION_MAX_ATTEMPTS", "1"))
        if max_attempts < 1:
            max_attempts = 1
    except Exception:
        max_attempts = 1
    ...
    while attempt < max_attempts:
        ...
        try:
            ...
            logging.warning(
                "Summarization attempt %s/%s timed out after %s seconds, %s",
                attempt,
                max_attempts,
                timeout_seconds,
                "giving up and returning original content" if attempt >= max_attempts else "retrying",
            )
            ...
            logging.warning(
                "Summarization failed with error on attempt %s/%s: %s, returning original content",
                attempt,
                max_attempts,
                str(e),
            )
...
def get_api_key_for_model(model_name: str, config: RunnableConfig):
    ...
    Preference order:
    1. Keys provided via config["configurable"]["apiKeys"].
...
def get_tavily_api_key(config: RunnableConfig):
    ...
    Preference order:
    1. Keys provided via config["configurable"]["apiKeys"].
    2. Environment variable TAVILY_API_KEY as a fallback.
    """
    api_keys = None
    if config is not None:
        api_keys = config.get("configurable", {}).get("apiKeys", {})
